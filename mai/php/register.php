...
if($num != 0){
    ...
    $q = "insert into users(userName,sex,email,birthday,pass,securityProblem,answer)
            VALUES ('{$_GET['name']}','{$_GET['sex']}','{$_GET['email']}',
            '{$_GET['birthday']}','{$_GET['pass']}','{$_GET['securityProblem']}',
            '{$_GET['answer']}');";

    $r = mysqli_query($conn,$q);
    if($r){
        setcookie('userName',$_GET['name'],time()+1800,"/");
        $con = mysqli_connect("localhost", "root", "oracle", "database", "3306");
        $q = "select user_id FROM users WHERE userName = '{$_GET['name']}';";
